...
public class KakaoOauthService implements OauthService<OauthLoginRequestDTO> {
    ...
    public OauthLoginResponseDTO login(OauthLoginRequestDTO param) {
        ...
        KakaoUserInfo userFromKakao = getUserInfoFromKakao(param.provider(), oauthToken, provider);
        String oauthProviderUniqueKey = userFromKakao.getProviderId();
        boolean isRegistered = userRepository.existsUserByOauthProviderUniqueKey(oauthProviderUniqueKey);
        return userFromKakao.toDTO(isRegistered);
    }
    ...
    private KakaoUserInfo getUserInfoFromKakao(String providerName, OauthTokenResponseDTO oauthToken,
